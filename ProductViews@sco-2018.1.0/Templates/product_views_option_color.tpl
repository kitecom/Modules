	...
	<div class="{{cartOptionId}}-controls-group" data-validation="control-group">
		{{#if showLabel}}
			<div class="product-views-option-color-label-header">
				<!--<a href="#" data-toggle="" data-target="#{{cartOptionId}}" aria-expanded="false" aria-controls="{{cartOptionId}}">-->
		...
		{{/if}}
		<div id="{{cartOptionId}}" class=" {{cartOptionId}}-controls product-views-option-color-container {{#if showSmall}}product-views-option-color-container-small{{/if}}" data-validation="control">
			{{#each values}}
				{{#if internalId}}
